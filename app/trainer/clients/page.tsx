...
function calculateRemainingSessions(
  ...
  if (savedRemaining !== null) return savedRemaining;
...
function calculateUsedSessions(
  ...
  if (savedUsed !== null) return savedUsed;

  return Math.max(totalSessions - remainingSessions, 0);
}

function TrainerClientsPageContent() {
    ...
    return clients
      ...
      .map((client) => {
        ...
        const paidPurchases = clientPurchases.filter(
          (purchase) =>
            purchase.status === "paid" || purchase.status === "confirmed"
        );
        ...
        return {
          ...
          assignedNutritionCoach: client.assigned_nutrition_coach_id
            ? staffNameMap.get(client.assigned_nutrition_coach_id) ||
              "Unknown NC"
            : "Not assigned",
          purchaseDate:
          ...
          purchaseType: getPurchaseTypeLabel(
            latestPurchase?.purchase_type || null
          ),
          status: getStatusLabel(client.status, remainingSessions),
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr_260px]">
              <div>
                ...
                <input
                  ...
                  placeholder="Search code, client, PT, NC, package, type, source, status..."
                ...
                <table className="w-full min-w-[1790px] table-fixed border-collapse text-left text-xs">
                  <thead>
                    <tr className="bg-yellow-400 text-black">
                      ...
                      <th className="w-[110px] px-3 py-3 text-right text-xs font-semibold uppercase">
                        Action
                      </th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {filteredAndSortedRows.map((row, index) => (
                      <tr
                        key={row.clientId || `${row.clientCode}-${index}`}
                        ...
                        <td className="px-3 py-3 text-right">
                          {row.clientId ? (
                            ...
                            <button
                              type="button"
                              disabled
                              className="cursor-not-allowed rounded-md border border-red-400 px-3 py-1.5 text-xs font-semibold uppercase text-red-300 opacity-70"
                            >
                              Missing ID
                            </button>
                          )}
