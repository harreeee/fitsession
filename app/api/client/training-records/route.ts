  ...
  new Set(
    ...
    const [sessionLogsResult, sessionHistoryResult] = await Promise.all([
      supabaseAdmin
        .from("session_logs")
        .select(
          "id, client_id, trainer_id, status, message, remaining_after, scanned_at"
        )
    ...
    const sessionLogsRows = sessionLogsResult.error
      ? []
      : ((sessionLogsResult.data || []) as SessionLogRow[]);

    const sessionHistoryRows = sessionHistoryResult.error
      ? []
      : ((sessionHistoryResult.data || []) as SessionHistoryRow[]);

    const mergedLogs = [
      ...sessionLogsRows.map((log) => ({
        id: log.id,
        ...
        created_at: log.scanned_at || new Date().toISOString(),
        source: "session_logs",
      })),

      ...sessionHistoryRows.map((log) => ({
        id: log.id,
        client_id: log.client_id,
        trainer_id: log.trainer_id,
        status: log.status || "success",
...
lookup_client_ids: lookupClientIds,
        session_logs_count: sessionLogsRows.length,
        session_history_count: sessionHistoryRows.length,
        returned_count: logs.length,
        session_logs_error: sessionLogsResult.error?.message || null,
        session_history_error: sessionHistoryResult.error?.message || null,
      },
    });
  } catch (error) {
    const message =
