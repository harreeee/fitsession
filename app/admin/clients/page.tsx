...
function calculatePackageValue(
  ...
  return (
    toNumber(latestPackage?.package_value) ?? toNumber(latestPurchase?.price)
  );
}
...
export default function AdminClientsPage() {
  ...
  const showAddClientButton = canAddClients(userRole);
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                {showAddClientButton && (
                  <Link
                    ...
                    className="rounded-xl bg-green-400 px-4 py-2 text-center text-xs font-semibold uppercase text-black transition hover:bg-green-300"
                  >
                    Add Client
                  </Link>
                )}
            ...
            <section className="overflow-hidden rounded-2xl border border-yellow-500/30 bg-black/65 shadow-2xl">
              <div className="border-b border-yellow-500/30 bg-black px-4 py-3">
                <p className="text-xs font-normal uppercase tracking-widest text-yellow-400">
                  Showing {filteredAndSortedRows.length} of {tableRows.length}{" "}
                  clients
                </p>
              </div>
