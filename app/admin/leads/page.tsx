...
export default function AdminLeadsPage() {
  ...
  }, [leads, search, statusFilter, sourceFilter, staffMap]);
        ...
        <header className="mb-6 rounded-3xl border border-yellow-500/30 bg-white/[0.06] p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Link href="/admin" className="rounded-xl bg-yellow-400 px-4 py-2 text-sm font-semibold text-black">Back to Admin</Link>
            </div>
          </div>
        </header>
          ...
          {loading ? <p className="py-8 text-center text-yellow-400">Loading leads...</p> : (
            <div className="mt-5 space-y-4">
              {filteredLeads.map((lead) => (
                <div key={lead.id} className="rounded-3xl border border-white/10 bg-black/45 p-5">
                  <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr_0.9fr]">
                    <div><h3 className="text-xl font-semibold text-white">{lead.full_name}</h3><p className="mt-1 text-sm text-gray-400">{lead.phone || "No phone"} · {lead.email || "No email"}</p><p className="mt-2 text-sm text-yellow-300">{sourceLabel(lead.source_type, lead.source_detail)}</p>{lead.notes ? <p className="mt-2 text-sm text-gray-300">{lead.notes}</p> : null}</div>
                    <div><label className="text-xs uppercase text-gray-400">Demo date</label><input type="datetime-local" defaultValue={toDateTimeLocal(lead.demo_at)} onBlur={(e) => updateLead(lead.id, { demo_at: e.target.value || null, status: e.target.value ? "demo_booked" : lead.status })} className="mt-2 w-full rounded-xl border border-white/15 bg-black/70 px-3 py-2" /><label className="mt-3 block text-xs uppercase text-gray-400">Assigned PT</label><select value={lead.assigned_trainer_id || ""} onChange={(e) => updateLead(lead.id, { assigned_trainer_id: e.target.value || null })} className="mt-2 w-full rounded-xl bg-white px-3 py-2 text-black"><option value="">Unassigned</option>{staff.map((person) => <option key={person.id} value={person.id}>{person.full_name || person.email}</option>)}</select></div>
                    <div><label className="text-xs uppercase text-gray-400">Status</label><select value={lead.status} onChange={(e) => updateLead(lead.id, { status: e.target.value as LeadStatus })} className="mt-2 w-full rounded-xl bg-white px-3 py-2 text-black">{STATUS_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><button type="button" onClick={() => convertLead(lead)} disabled={convertingId === lead.id || lead.status === "lost"} className="mt-4 w-full rounded-xl bg-green-400 px-4 py-3 font-semibold uppercase text-black disabled:opacity-50">{lead.converted_client_id ? "Open Client" : convertingId === lead.id ? "Converting..." : "Convert to Client"}</button></div>
                  </div>
                </div>
              ))}
